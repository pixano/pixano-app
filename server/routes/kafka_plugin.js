...
const getIdsInputListFromKafka = async () => {
	...
	const kafka = new Kafka({
		clientId: 'PIXANO',
		brokers: ['kafka.ec5-dev.svc.cluster.local:9092'],
		retry: {
			retries: 3,
			restartOnFailure: false
		}
	});
	var topicName = "selection";
	const kafkaConsumer = kafka.consumer({ groupId: 'confiance-group' });

	// 2) get ids
	var sample_ids = [];
	await kafkaConsumer.connect().catch((e) => { throw e; });
	console.log("connected ok");
	await kafkaConsumer.subscribe({ topic: topicName, fromBeginning: false}).catch((e) => { throw e; });
