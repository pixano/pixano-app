...
const downloadFilesFromMinio = async (listIds) => {
	...
	const bucket_name = 'pixanoimagesselection';
	...
	const promesse = new Promise((resolve, reject) => {
		...
		stream.on('data', function (obj) {
			...
			if (listOfURLs.length===0) reject("Minio: no data found in bucket "+bucket_name);
			resolve(listOfURLs);
		});
	...
	if (listOfURLs.length===0) throw "Minio: no data found in bucket "+bucket_name;
